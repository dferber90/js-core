...
interface VercelStorage {
  [namespace: string]: {
    // TODO: Consider export of flag and segment type.
    [key: string]: any;
  };
}
...
class VercelFeatureStore implements LDFeatureStore {
  ...
  async get(
    ...
    try {
      // TODO: Does this return a copy or the same thing in memory? If it isn't a copy
      // then we may process flags that have already been processed. This may be ok.
      ...
      const item = config as VercelStorage;
      if (kind.namespace === 'features') {
        callback(processFlag(item[kindKey][flagKey]));
      } else if (kind.namespace === 'segments') {
        callback(processSegment(item[kindKey][flagKey]));
      }
    } catch (err) {
  ...
  async all(kind: DataKind, callback: (res: LDFeatureStoreKindData) => void = noop): Promise<void> {
    ...
    try {
      ...
      const items = config as VercelStorage;
      callback(
        items[kindKey].forEach((item: any) => {
          if (kind.namespace === 'features') {
            processFlag(item);
          }
          if (kind.namespace === 'segments') {
            processSegment(item);
          }
        })
      );
